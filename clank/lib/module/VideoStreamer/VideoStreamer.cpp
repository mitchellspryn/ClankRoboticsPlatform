...
VideoStreamer::VideoStreamer(Clank::Configuration::Configuration *config)
{
  string mjpegStreamerPath;

  if (!config->GetConfigurationData("MjpegStreamerPath", mjpegStreamerPath))
  {
    string errorMessage = "VideoStreamer error: invalid configuration.\n";
    errorMessage += "Configuration: <MjpegStreamerPath>\n";
    throw runtime_error(errorMessage);
  }

  this->MjpgStreamerPath = mjpegStreamerPath;
...
void VideoStreamer::Stream()
{
  ...
  system((this->MjpgStreamerPath + "start.sh").c_str());
  while (1)
  {
    DEBUG_MSG(".");
    sleep(1);
